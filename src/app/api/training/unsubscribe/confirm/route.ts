...
import { confirmUnsubscribeByToken } from '@/lib/training/unsubscribe'
...
export async function POST(request: NextRequest) {
  ...
  if (!checkRateLimit(`unsubscribe-confirm:${ip}`, UNSUBSCRIBE_RATE_LIMIT, UNSUBSCRIBE_RATE_WINDOW_MS)) {
    return NextResponse.json({ error: 'Too many requests from this connection. Please try again later.' }, { status: 429 })
  }

  const ok = await confirmUnsubscribeByToken(token)
  if (!ok) {
