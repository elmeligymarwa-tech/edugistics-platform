...
/** Course name only — never the teacher's name, email, phone, school, subject, grade or registration reference. Meta already matches the browser to a profile via its own cookie; nothing submitted on the form is handed over on top of that. */
export function trackCompleteRegistration(courseName: string): void {
  callFbq('track', 'CompleteRegistration', { course_name: courseName })
}
 ...
 */
export function trackJoinedWaitlist(courseName: string): void {
  callFbq('trackCustom', 'JoinedWaitlist', { course_name: courseName })
}
