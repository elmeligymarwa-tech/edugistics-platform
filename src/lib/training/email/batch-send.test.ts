// Unit tests for mapBatchResponseToOutcomes — the piece of the batching
// design most likely to silently misattribute a send outcome to the wrong
// recipient if the index bookkeeping is wrong. Pure function, no Prisma, no
// Resend client, no database: safe to run via `npm run test:batch-send`
// (vitest.batch-send.config.mts), which has no globalSetup.
import { describe, expect, it } from 'vitest'

import { mapBatchResponseToOutcomes } from './batch-send'

const recipients = [{ recipientId: 'r1' }, { recipientId: 'r2' }, { recipientId: 'r3' }, { recipientId: 'r4' }]
...
describe('mapBatchResponseToOutcomes', () => {
  it('maps every recipient to its message id when nothing fails', () => {
    ...
    expect(outcomes).toEqual([
      { recipientId: 'r1', ok: true, messageId: 'm1' },
      { recipientId: 'r2', ok: true, messageId: 'm2' },
      { recipientId: 'r3', ok: true, messageId: 'm3' },
      { recipientId: 'r4', ok: true, messageId: 'm4' },
    ])
  })
  ...
  it('attributes a validation error to the recipient at that original index, not by position in data', () => {
    ...
    expect(outcomes).toEqual([
      { recipientId: 'r1', ok: true, messageId: 'm1' },
      { recipientId: 'r2', ok: false, error: 'Invalid `to` field' },
      { recipientId: 'r3', ok: true, messageId: 'm3' },
      { recipientId: 'r4', ok: true, messageId: 'm4' },
    ])
  })
  ...
  it('handles a failure at the first index without shifting every later recipient off by one', () => {
    ...
    expect(outcomes[0]).toEqual({ recipientId: 'r1', ok: false, error: 'Invalid `to` field' })
    expect(outcomes[1]).toEqual({ recipientId: 'r2', ok: true, messageId: 'm2' })
    expect(outcomes[3]).toEqual({ recipientId: 'r4', ok: true, messageId: 'm4' })
  })
  ...
  it('fails a recipient explicitly rather than crashing if data runs out unexpectedly', () => {
    ...
    expect(outcomes[0]).toEqual({ recipientId: 'r1', ok: true, messageId: 'm1' })
