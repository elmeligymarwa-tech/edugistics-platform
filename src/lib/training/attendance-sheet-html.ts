...
import { formatAdminTimestamp, formatCourseDateOrRange } from '@/domain/training/format'
...
export function buildAttendanceSheetHtml(data: AttendanceSheetData): string {
  const { course, rows, includeWaitlisted } = data
  const dateLabel = formatCourseDateOrRange(course)
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      th, td { border: 1px solid #c7ccd9; padding: 6px 8px; text-align: left; vertical-align: middle; }
      th { background: #eef1f7; font-weight: 600; }
      tr { break-inside: avoid; page-break-inside: avoid; }
      td.mono { font-family: ui-monospace, Menlo, Consolas, monospace; }
      td.box { width: 60px; text-align: center; }
      td.box::after {
        content: '';
  ...
  <body>
    ...
    <table>
      ...
      <tbody>
        ${rowsHtml || '<tr><td colspan="6">No registrations to list.</td></tr>'}
      </tbody>
    </table>

    <p class="print-hint">Use your browser's Print (Ctrl/Cmd+P) to print or save as PDF — this page is formatted for A4.</p>
  </body>
</html>`
}
